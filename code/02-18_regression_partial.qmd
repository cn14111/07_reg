...
#| message: false
#| warning: false
ggplot(data = reg_dfw, aes(x = sr_ksha, y= yield_mgha, color = sr_ksha)) + geom_boxplot()+ geom_jitter() + theme(legend.position = "none")
```

What is going on with this boxplot?
it is considering seeding rate as numerical variable

if interested in means we need to change seeding rate as factor then we will get box plot for each seeding rate
...
```
looks like quadratic plot <- "interpret the plot"
...
```
p value is highly significant doesn't mean it is a good or bad model. we need to check the residuals.
## b) Model Assumptions
```{r mod1 augmenting}
...
standardised residuals are used to assess outliers
for consistency resid function works instead of rstudent(which we used in RCBD )

leftjoin is used to bring back seeding rate column from the reg_dfw and join in the mod1_int
...
Increasing pattern of residuals. Model is underpredicting at higher fitted values(higher seeding rate) and overpredicting at lower fitted values(lower seeding rate).

This plot stating try add a **slope** to the model, it will improve the model
...
**bootstrap function** is used for resampling the data points with replacement. Creating different realizations from the data and extracting coefficients from them.

pull the samples from a given sample <- resample

create a dataframe with seeding rate between min(40) and max(120) (from 40 to 120 as step of 1)
...
nd
```
want to give all the seeding rates to model and then ask model to predict the yield.
model predicts the intercept regardless of the seeding rate 
as it is a random effect model, we need to specify what level we need to predict

nsim = 500 to 1000 is the good number to get a better estimates of the values
...
mod1_int_boots <- bootMer(mod1_int, 
                          predict.fun, 
                          nsim = 200) %>% #bootstrapping 200 times (nsim gives the number of simulations we want)
  confint() %>%
  as.data.frame() %>%
...
```
81 rows (are the seeding rate we created above)
...
```
sr_ksha <- 0.05195 (slope coefficient) - every unit change in yield
...
one way of fixing residuals is fixing model
...
in the above summary, intercept value is 8.01841 but when we plot it shows 10,(starting from zero) therefore we use range (scale_x_continuous(limits = c(0, 120))
after using the function , we can see a change (intercept is seen at 8)
